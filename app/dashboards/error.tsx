...
export default function Error({
  ...
  useEffect(() => {
    if (process.env.NODE_ENV === 'production' || process.env.NEXT_PUBLIC_LOG_ERRORS === 'true') {
      logger.error('Dashboards error boundary caught error', error, {
        module: 'DashboardsErrorBoundary',
        digest: error.digest,
      });
    }
    ...
    if (process.env.NEXT_PUBLIC_SENTRY_DSN) {
      Sentry.captureException(error, {
        level: 'error',
        tags: { module: 'DashboardsErrorBoundary' },
  ...
  return (
    <div className="min-h-[calc(100vh-64px)] flex items-center justify-center bg-fcCream p-4">
      <div className="max-w-md w-full p-6 bg-white rounded-lg shadow-lg border border-fcDivider">
        <h2 className="text-2xl font-bold text-fcSlate mb-3">Dashboard error</h2>
