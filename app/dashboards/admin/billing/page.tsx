 ...
 * Unified payment ledger, allocations, and reconciliation for Admins
 */
export default function BillingPage() {
  ...
  useEffect(() => {
    ...
    if (status === 'unauthenticated' || !session?.user) {
      router.replace('/login?callbackUrl=/dashboards/admin/billing');
      return;
    }
    ...
    (async () => {
      try {
        ...
        console.error('[BillingPage] Auth error:', e);
        router.replace('/login?callbackUrl=/dashboards/admin/billing');
      }
    })();
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center">
          ...
          <p className="mt-4 text-gray-600 font-medium">Loading Billing Dashboard…</p>
        </div>
      </div>
    );
  }
  ...
  return <BillingModule activeBranch={branch} />;
}
