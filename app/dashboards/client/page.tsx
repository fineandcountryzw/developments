...
const ClientDashboard = dynamic(
  () => import('@/components/dashboards/ClientDashboard').then(mod => mod.ClientDashboard),
  { ssr: false }
);
...
export default function ClientDashboardPage() {
  ...
  useEffect(() => {
    ...
    if (status === 'unauthenticated' || !session?.user) {
      router.replace('/login?callbackUrl=/dashboards/client');
      return;
    }
    ...
    (async () => {
      try {
        ...
        if (!res.ok || !json?.success) {
          console.warn('[ClientPage] /api/auth/me failed:', json?.error ?? res.status);
          router.replace('/login?callbackUrl=/dashboards/client');
          return;
        }
        ...
        console.log('[ClientPage] DB role:', user.role, 'Normalized:', normalizedRole, 'email:', user.email);

        // Use centralized access control
        if (normalizedRole && canUserAccessDashboard(user, 'client')) {
          setCanRender(true);
          return;
        }
        ...
        console.log('[ClientPage] Access denied, redirecting to:', redirectPath);
        router.replace(redirectPath);
      } catch (e) {
        console.error('[ClientPage] /api/auth/me error:', e);
        router.replace('/login?callbackUrl=/dashboards/client');
      }
    })();
