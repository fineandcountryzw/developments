...
const AgentDashboard = dynamic(
  () => import('@/components/dashboards/AgentDashboard').then(mod => mod.AgentDashboard),
  { 
    ssr: false,
    loading: () => (
      <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900">
        <div className="text-center space-y-4">
          ...
          <p className="text-white font-semibold">Loading Agent Dashboard...</p>
        </div>
      </div>
    )
  }
...
export default function AgentDashboardPage() {
  ...
  useEffect(() => {
    ...
    if (status === 'unauthenticated' || !session?.user) {
      router.replace('/login?callbackUrl=/dashboards/agent');
      return;
    }
    ...
    (async () => {
      try {
        ...
        if (!res.ok || !json?.success) {
          console.warn('[AgentPage] /api/auth/me failed:', json?.error ?? res.status);
          router.replace('/login?callbackUrl=/dashboards/agent');
          return;
        }
        ...
        console.log('[AgentPage] DB role:', user.role, 'Normalized:', normalizedRole, 'email:', user.email);

        // Use centralized access control
        if (normalizedRole && canUserAccessDashboard(user, 'agent')) {
          setCanRender(true);
          return;
        }
        ...
        console.log('[AgentPage] Access denied, redirecting to:', redirectPath);
        router.replace(redirectPath);
      } catch (e) {
        console.error('[AgentPage] /api/auth/me error:', e);
        router.replace('/login?callbackUrl=/dashboards/agent');
      }
    })();
