...
import { 
  ArrowRight, Building2, MapPin, Layers, Droplets, Landmark, 
  Clock, ShieldCheck, FileText, Phone, Mail, Globe, Lock, 
  Cpu, Database, CheckCircle2, Layout, Zap, Fingerprint,
  Briefcase, User, Users, TrendingUp, Route, Sun, Wifi, Hammer, Shield,
  ShieldAlert, Cpu as ChipIcon, FileCode,
  ChevronRight, Camera, Maximize2, Download, TrendingUp as AppreciationIcon,
  Check, Waves, ChevronLeft, ImageIcon, MapPinOff, X, Calendar
} from 'lucide-react';
import { Role, Development, Stand, DevelopmentPhase, DevelopmentDocument, Agent } from '../types';
import { getDevelopments, getStandsByDevelopment, getAgents } from '../lib/db';
...
const PlotSelectorMap = dynamic(
  ...
  { ssr: false, loading: () => <div className="w-full h-96 bg-gray-100 flex items-center justify-center"><span>Loading map...</span></div> }
);
...
const ProgressBar = ({ label, percentage, icon: Icon }: { label: string; percentage: number; icon: any }) => (
  <div className="space-y-3 font-sans">
    <div className="flex justify-between items-center">
      <div className="flex items-center space-x-3">
        <div className="p-2 bg-fcGold/10 rounded-lg text-fcGold"><Icon size={14} /></div>
        <span className="text-[10px] font-bold text-fcSlate uppercase tracking-widest">{label}</span>
      </div>
      <span className="text-[11px] font-black text-fcGold font-mono">{percentage}%</span>
    </div>
...
const ImageGallery: React.FC<ImageGalleryProps> = ({ images, developmentName }) => {
    ...
    <div className="space-y-4">
      ...
      >
        <Image
          ...
          unoptimized={!validImages[currentIndex]?.includes('ufs.sh') && !validImages[currentIndex]?.includes('supabase.co')}
          onError={(e) => {
          ...
          {validImages.slice(0, 6).map((img, idx) => (
            ...
            >
              <Image
                ...
                unoptimized={!img?.includes('ufs.sh') && !img?.includes('supabase.co')}
                onError={(e) => {
                  const target = e.target as HTMLImageElement;
          ...
          {validImages.map((_, idx) => (
            <button
              ...
              className={`w-2 h-2 rounded-full transition-all ${
                idx === currentIndex ? 'bg-fcGold w-6' : 'bg-gray-300'
              }`}
...
export const LandingPage: React.FC<LandingPageProps> = () => {
  ...
  const [phaseFilter, setPhaseFilter] = useState<'ALL' | DevelopmentPhase>('ALL');
  ...
  const [selectedStandForReservation, setSelectedStandForReservation] = useState<any>(null);
  ...
  useEffect(() => {
    ...
    cachedFetch<any>('/api/admin/developments')
        ...
        const validDevs = Array.isArray(devs) ? devs.map((dev: any) => {
          ...
          try {
            if (dev.standSizes || dev.stand_sizes) {
              ...
              parsedStandSizes = typeof sizes === 'string' 
                ? JSON.parse(sizes) 
                : sizes;
            }
          } catch (e) {
          ...
          return {
            ...
            installmentPeriods: (() => {
              if (Array.isArray(dev.installmentPeriods || dev.installment_periods)) {
                return (dev.installmentPeriods || dev.installment_periods).filter((p: any) => typeof p === 'number' && p > 0);
              }
              if (typeof (dev.installmentPeriods || dev.installment_periods) === 'string') {
                try {
                  ...
                  if (Array.isArray(parsed)) {
                    return parsed.filter((p: any) => typeof p === 'number' && p > 0);
                  }
                } catch (e) {
                  // Invalid JSON
                }
            ...
            basePrice: typeof (dev.basePrice ?? dev.base_price) === 'number' ? (dev.basePrice ?? dev.base_price) : parseFloat(String((dev.basePrice ?? dev.base_price) || 0)) || 0,
            vatPercentage: typeof (dev.vatPercentage ?? dev.vat_percentage) === 'number' ? (dev.vatPercentage ?? dev.vat_percentage) : parseFloat(String((dev.vatPercentage ?? dev.vat_percentage) || 15.5)) || 15.5,
            endowmentFee: typeof (dev.endowmentFee ?? dev.endowment_fee) === 'number' ? (dev.endowmentFee ?? dev.endowment_fee) : parseFloat(String((dev.endowmentFee ?? dev.endowment_fee) || 0)) || 0,
            depositPercentage: typeof (dev.depositPercentage ?? dev.deposit_percentage) === 'number' ? (dev.depositPercentage ?? dev.deposit_percentage) : parseFloat(String((dev.depositPercentage ?? dev.deposit_percentage) || 10)) || 10,
            // Ensure boolean fields are booleans
            vatEnabled: typeof (dev.vatEnabled ?? dev.vat_enabled) === 'boolean' ? (dev.vatEnabled ?? dev.vat_enabled) : (dev.vatEnabled ?? dev.vat_enabled) !== undefined ? Boolean(dev.vatEnabled ?? dev.vat_enabled) : true,
            endowmentEnabled: typeof (dev.endowmentEnabled ?? dev.endowment_enabled) === 'boolean' ? (dev.endowmentEnabled ?? dev.endowment_enabled) : (dev.endowmentEnabled ?? dev.endowment_enabled) !== undefined ? Boolean(dev.endowmentEnabled ?? dev.endowment_enabled) : true,
            aosEnabled: typeof (dev.aosEnabled ?? dev.aos_enabled) === 'boolean' ? (dev.aosEnabled ?? dev.aos_enabled) : (dev.aosEnabled ?? dev.aos_enabled) !== undefined ? Boolean(dev.aosEnabled ?? dev.aos_enabled) : false,
            cessionsEnabled: typeof (dev.cessionsEnabled ?? dev.cessions_enabled) === 'boolean' ? (dev.cessionsEnabled ?? dev.cessions_enabled) : (dev.cessionsEnabled ?? dev.cessions_enabled) !== undefined ? Boolean(dev.cessionsEnabled ?? dev.cessions_enabled) : false,
          };
        }) : [];
      ...
      .catch(err => {
        ...
        logger.error('Error fetching developments', err, { module: 'LandingPage' });
    ...
    cachedFetch<{ data: Agent[]; error?: string }>('/api/admin/agents')
      .then(data => {
        ...
        logger.debug('Fetched agents', { module: 'LandingPage', count: agentList.length });
      })
      .catch(err => {
  ...
  useEffect(() => {
    const handleReserveStandEvent = (event: Event) => {
      ...
      const standData = {
        ...
        pricePerSqm: selectedDev?.pricePerSqm || (selectedDev?.basePrice && selectedDev?.totalAreaSqm ? selectedDev.basePrice / selectedDev.totalAreaSqm : undefined),
  ...
  useEffect(() => {
    const handleOpenModal = async (event: any) => {
      ...
      try {
        ...
        logger.error('Forensic lead capture error (non-blocking)', err instanceof Error ? err : undefined, { module: 'LandingPage' });
      }
  ...
  const filteredDevelopments = useMemo(() => {
    ...
    return developments.filter(d => d && (phaseFilter === 'ALL' || d.phase === phaseFilter));
  ...
  const calculateTotal = (dev: Development) => {
    ...
    const vatEnabled = dev.vatEnabled !== undefined ? dev.vatEnabled : (dev as any)?.vatEnabled !== undefined ? (dev as any).vatEnabled : true;
    const endowmentEnabled = dev.endowmentEnabled !== undefined ? dev.endowmentEnabled : (dev as any)?.endowmentEnabled !== undefined ? (dev as any).endowmentEnabled : true;
    const aosEnabled = dev.aosEnabled !== undefined ? dev.aosEnabled : (dev as any)?.aosEnabled !== undefined ? (dev as any).aosEnabled : false;
    const aosFee = typeof dev.aosFee === 'number' 
      ? dev.aosFee 
      : typeof (dev as any)?.aosFee === 'string'
        ? parseFloat((dev as any).aosFee) || 500
        : 500;
    const cessionsEnabled = dev.cessionsEnabled !== undefined ? dev.cessionsEnabled : (dev as any)?.cessionsEnabled !== undefined ? (dev as any).cessionsEnabled : false;
    const cessionFee = typeof dev.cessionFee === 'number' 
      ? dev.cessionFee 
      : typeof (dev as any)?.cessionFee === 'string'
        ? parseFloat((dev as any).cessionFee) || 250
        : 250;

    const adminFeeEnabled = (dev as any)?.adminFeeEnabled !== undefined
      ? (dev as any).adminFeeEnabled
      : false;
    const adminFee = typeof (dev as any)?.adminFee === 'number'
      ? (dev as any).adminFee
    ...
    const vatAmount = vatEnabled ? subtotalBeforeVat * (vatPercentage / 100) : 0;
  ...
  const handleConfirmReservation = async (standId: string, reservationData: any) => {
    ...
    alert(`Reservation confirmed for Stand ${standId}! Digital Ref: ${reservationData.digitalRef}`);
  };
    ...
    <div className="relative min-h-screen flex flex-col bg-white overflow-x-hidden font-sans">
      ...
      {loadError && (
        <div className="fixed top-20 left-4 right-4 max-w-md z-50 bg-red-50 border border-red-200 rounded-lg p-4 shadow-lg">
          <p className="text-sm font-semibold text-red-800">Error loading developments</p>
      ...
      <main className="flex-1 pt-16 md:pt-20">
        ...
        {!selectedDev && (
          <section className="relative px-4 md:px-6 lg:px-12 pt-32 md:pt-40 pb-20 md:pb-32 bg-white overflow-hidden min-h-[600px]">
            {/* Animated Background Gradients */}
            <HeroAnimatedBackground />
            
            {/* Content Layer */}
            <div className="relative z-10 max-w-7xl mx-auto">
              <div className="text-center space-y-8 md:space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
                {/* Hero Badge */}
                <div className="inline-flex items-center gap-2 px-4 py-2 bg-fcGold/10 border border-fcGold/20 rounded-full">
                  ...
                  <span className="text-xs font-bold text-fcGold uppercase tracking-widest">Premium Property Platform</span>
                </div>
                
                {/* Hero Headline */}
                <div className="space-y-6">
                  <h1 className="text-4xl md:text-5xl lg:text-[48px] font-semibold text-fcSlate tracking-tight leading-[1.2] max-w-4xl mx-auto">
                    Secure Your Future with
                    <span className="block text-fcGold mt-2">Premium Land Investments</span>
                  </h1>
                  <p className="text-base md:text-lg text-gray-600 max-w-2xl mx-auto leading-[1.6] font-normal">
                    Verified developments, transparent pricing, and secure transactions. Reserve your stand with confidence.
                  </p>
                </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-6 pt-12 max-w-3xl mx-auto">
                  <div className="text-center">
                    <div className="text-3xl md:text-4xl font-black text-fcGold mb-2">72h</div>
                    <div className="text-xs font-bold text-gray-600 uppercase tracking-wider">Exclusive Hold</div>
                  </div>
                  <div className="text-center">
                    <div className="text-3xl md:text-4xl font-black text-fcGold mb-2">100%</div>
                    <div className="text-xs font-bold text-gray-600 uppercase tracking-wider">Secure Payments</div>
                  </div>
                  <div className="text-center">
                    <div className="text-3xl md:text-4xl font-black text-fcGold mb-2">✓</div>
                    <div className="text-xs font-bold text-gray-600 uppercase tracking-wider">Verified Developments</div>
                  </div>
                  <div className="text-center">
                    <div className="text-3xl md:text-4xl font-black text-fcGold mb-2">24/7</div>
                    <div className="text-xs font-bold text-gray-600 uppercase tracking-wider">Support</div>
                  </div>
                </div>
              </div>
            </div>
        ...
        {!selectedDev && (
          <>
            ...
            <section className="px-4 md:px-6 lg:px-12 py-16 md:py-24 bg-white">
              <div className="max-w-7xl mx-auto">
                <div className="text-center mb-12 md:mb-16">
                  <h2 className="text-3xl md:text-[32px] font-semibold text-fcSlate mb-4 tracking-tight leading-[1.25]">Why Trust Us</h2>
                  <p className="text-base text-gray-600 max-w-2xl mx-auto leading-[1.6]">Built for transparency, security, and your peace of mind</p>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8">
                  <div className="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm hover:shadow-md transition-shadow duration-150 ease-out focus-within:ring-2 focus-within:ring-fcGold focus-within:ring-offset-2">
                    ...
                    <h3 className="text-xl font-semibold text-fcSlate mb-3 leading-[1.3]">Secure Transactions</h3>
                    <p className="text-gray-600 leading-[1.6] text-base">
                      All payments are encrypted and processed through verified banking channels. Your financial data is protected with bank-level security.
                    </p>
                  </div>
                  <div className="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm hover:shadow-md transition-shadow duration-150 ease-out focus-within:ring-2 focus-within:ring-fcGold focus-within:ring-offset-2">
                    ...
                    <h3 className="text-xl font-semibold text-fcSlate mb-3 leading-[1.3]">Verified Developments</h3>
                    <p className="text-gray-600 leading-[1.6] text-base">
                      Every development undergoes rigorous verification. We verify legal status, title deeds, and development approvals before listing.
                    </p>
                  </div>
                  <div className="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm hover:shadow-md transition-shadow duration-150 ease-out focus-within:ring-2 focus-within:ring-fcGold focus-within:ring-offset-2">
                    ...
                    <h3 className="text-xl font-semibold text-fcSlate mb-3 leading-[1.3]">Transparent Pricing</h3>
                    <p className="text-gray-600 leading-[1.6] text-base">
                      Complete fee breakdown with VAT, admin fees, and all charges clearly displayed. No hidden costs, no surprises.
                    </p>
                  </div>
                </div>
              </div>
            ...
            <section id="process-timeline" className="px-4 md:px-6 lg:px-12 py-16 md:py-24 bg-gray-50/50">
              <div className="max-w-7xl mx-auto">
                <div className="text-center mb-12 md:mb-16">
                  <h2 className="text-3xl md:text-[32px] font-semibold text-fcSlate mb-4 tracking-tight leading-[1.25]">Simple Process</h2>
                  <p className="text-base text-gray-600 max-w-2xl mx-auto leading-[1.6]">From reservation to delivery in four clear steps</p>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6 md:gap-8">
                  {[
                    { step: '1', title: 'Reserve', desc: 'Select your stand and complete reservation', icon: MapPin },
                    { step: '2', title: 'Pay Deposit', desc: 'Secure your stand with deposit payment', icon: ShieldCheck },
                    { step: '3', title: 'Paperwork', desc: 'Complete legal documentation digitally', icon: FileText },
                    { step: '4', title: 'Delivery', desc: 'Receive title and take ownership', icon: CheckCircle2 },
                  ].map((item, idx) => (
                    <div key={idx} className="relative">
                      <div className="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm hover:shadow-md transition-shadow duration-150 ease-out text-center">
                        ...
                        <h3 className="text-xl font-semibold text-fcSlate mb-2 leading-[1.3]">{item.title}</h3>
                        <p className="text-base text-gray-600 leading-[1.6]">{item.desc}</p>
                      </div>
                      {idx < 3 && (
                        <div className="hidden md:block absolute top-1/2 left-full w-full h-0.5 bg-fcGold/20 -translate-y-1/2 -z-10" style={{ width: 'calc(100% - 2rem)' }}>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12 items-center">
                    <div>
                      <div className="inline-flex items-center gap-2 px-4 py-2 bg-blue-100 rounded-full mb-6">
                        ...
                        <span className="text-xs font-bold text-blue-900 uppercase tracking-wider">Bank-Level Security</span>
                      </div>
                      <h2 className="text-3xl md:text-[32px] font-semibold text-fcSlate mb-4 tracking-tight leading-[1.25]">Secure Payments & Protection</h2>
                      <p className="text-base text-gray-700 mb-6 leading-[1.6]">
                        Your transactions are protected with industry-leading security measures and compliance standards.
                      </p>
                      <div className="space-y-4">
                        <div className="flex items-start gap-3">
                          <CheckCircle2 size={20} className="text-blue-600 mt-0.5 flex-shrink-0" />
                          <div>
                            <p className="font-bold text-fcSlate text-sm">Encrypted Payment Processing</p>
                            <p className="text-xs text-gray-600">All financial data encrypted in transit and at rest</p>
                          </div>
                        </div>
                        <div className="flex items-start gap-3">
                          <CheckCircle2 size={20} className="text-blue-600 mt-0.5 flex-shrink-0" />
                          <div>
                            <p className="font-bold text-fcSlate text-sm">Verified Banking Channels</p>
                            <p className="text-xs text-gray-600">Direct integration with verified financial institutions</p>
                          </div>
                        </div>
                        <div className="flex items-start gap-3">
                          <CheckCircle2 size={20} className="text-blue-600 mt-0.5 flex-shrink-0" />
                          <div>
                            <p className="font-bold text-fcSlate text-sm">72-Hour Hold Guarantee</p>
                            <p className="text-xs text-gray-600">Your stand is locked exclusively to you during reservation</p>
                          </div>
                        </div>
                      </div>
                    </div>
                    <div className="bg-white rounded-2xl p-8 border border-gray-200 shadow-lg">
                      <h3 className="text-xl font-bold text-fcSlate mb-6">Payment Options</h3>
                      <div className="space-y-4">
                        ...
                        <div className="flex items-center justify-between p-4 bg-gray-50 rounded-xl">
                          <span className="font-medium text-fcSlate">Bank Transfer</span>
                          <CheckCircle2 size={20} className="text-green-600" />
                        </div>
                        <div className="flex items-center justify-between p-4 bg-gray-50 rounded-xl">
                          <span className="font-medium text-fcSlate">Mobile Money</span>
                          <CheckCircle2 size={20} className="text-green-600" />
                        </div>
                        <div className="flex items-center justify-between p-4 bg-gray-50 rounded-xl">
                          <span className="font-medium text-fcSlate">Installment Plans</span>
                          <CheckCircle2 size={20} className="text-green-600" />
                        </div>
                      </div>
                    </div>
        ...
        {/* Inventory showcase */}
        <section id="inventory" className={`px-4 md:px-6 lg:px-12 py-16 md:py-32 bg-white border-y border-fcDivider ${selectedDev ? 'pt-24 md:pt-40' : ''}`}>
        <div className="max-w-7xl mx-auto">
          {selectedDev ? (
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 space-y-12 md:space-y-16">
              ...
              <div className="border-b border-fcSlate/10 pb-8 md:pb-10">
                 <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4 md:gap-8">
                    <div className="space-y-3 min-w-0 flex-1">
                       <div className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-[10px] font-semibold uppercase tracking-wider ${
                          selectedDev?.phase === 'READY_TO_BUILD' 
                            ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' 
                            : 'bg-amber-50 text-amber-700 border border-amber-200'
                       }`}>
                          <span className={`w-1.5 h-1.5 rounded-full ${selectedDev?.phase === 'READY_TO_BUILD' ? 'bg-emerald-500' : 'bg-amber-500'}`}></span>
                          {selectedDev?.phase === 'READY_TO_BUILD' ? 'Ready to Build' : 'Servicing in Progress'}
                       </div>
                       <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold text-fcSlate tracking-tight">{selectedDev?.name || 'Development'}</h2>
                       <div className="flex items-center gap-2 text-fcSlate/60">
                          <MapPin size={14} className="text-fcGold" />
                          <span className="text-sm font-medium">{(selectedDev as any)?.locationName || (selectedDev as any)?.location || 'Location'}</span>
                       </div>
                    </div>
                 </div>
              </div>
              ...
              <div className="bg-white rounded-2xl border border-fcSlate/10 overflow-hidden">
                 <div className="px-6 py-4 border-b border-fcSlate/10 bg-fcSlate/[0.02]">
                    <div className="flex items-center gap-3">
                       ...
                       <h3 className="text-base font-semibold text-fcSlate">Overview</h3>
                    </div>
                 </div>
                 <div className="p-6">
                    <p className="text-[15px] md:text-base text-fcSlate/80 leading-relaxed">
                      {selectedDev?.overview || selectedDev?.description || "Overview coming soon. This development is currently being documented. Contact our team for more information."}
                    </p>
                 </div>
                 ...
                 {process.env.NODE_ENV === 'development' && (() => {
                   logger.debug('Development data', {
                     ...
                     hasFeatures: Array.isArray((selectedDev as any)?.features) && (selectedDev as any).features.length > 0,
                     featuresCount: (selectedDev as any)?.features?.length || 0,
                     pricePerSqm: (selectedDev as any)?.pricePerSqm,
                     imageUrls: selectedDev?.imageUrls,
                     phase: selectedDev?.phase,
                     hasGeoJSON: !!(selectedDev as any)?.geoJsonData || !!(selectedDev as any)?.geometry,
                   });
                   return null;
                 })()}
              </div>
              ...
              <div className="bg-white rounded-2xl border border-fcSlate/10 overflow-hidden">
                 <div className="px-6 py-4 border-b border-fcSlate/10 bg-fcSlate/[0.02]">
                    <div className="flex items-center gap-3">
                       ...
                       <div>
                          <h3 className="text-base font-semibold text-fcSlate">Gallery</h3>
                          <p className="text-xs text-fcSlate/50">Development visuals & site photography</p>
                       </div>
                    </div>
                 </div>
                 <div className="p-4">
              ...
              <div className="bg-white rounded-2xl border border-fcSlate/10 overflow-hidden">
                 <div className="px-6 py-4 border-b border-fcSlate/10 bg-fcSlate/[0.02]">
                    <div className="flex items-center gap-3">
                       ...
                       <div>
                          <h3 className="text-base font-semibold text-fcSlate">Interactive Map</h3>
                          <p className="text-xs text-fcSlate/50">Select your preferred stand</p>
                       </div>
                    </div>
                 </div>
                 <div className="p-4">
                 ...
                 {(() => {
                   ...
                   const hasDirectCoords = selectedDev?.latitude && selectedDev?.longitude;
                   const geoData = (selectedDev as any)?.geoJsonData;
                   const hasCenterCoords = geoData?.center?.lat && geoData?.center?.lng;
                   const hasGeoFeatures = geoData?.features?.length > 0;
                   const hasValidCoords = hasDirectCoords || hasCenterCoords || hasGeoFeatures;
                   ...
                   if (!hasValidCoords) {
                     return (
                       <div className="w-full aspect-video md:aspect-[16/9] lg:aspect-[1920/1080] bg-gray-100 rounded-2xl border border-fcDivider flex flex-col items-center justify-center text-gray-500">
                         <MapPinOff size={48} className="mb-3 opacity-50" />
                         <p className="text-sm font-semibold">Map not available for this development</p>
                         <p className="text-xs text-gray-400 mt-1">Location data is being finalized</p>
                       </div>
                     );
                   }
                   ...
                   const devWithGeoData = {
                     ...
                     longitude: selectedDev?.longitude || geoData?.center?.lng
                   };
                   ...
                   return (
                     ...
                     >
                       <PlotSelectorMap development={devWithGeoData as any} onReserve={handleReserve} />
                     </div>
                   );
                 })()}
                 </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 md:gap-12 lg:gap-16">
                ...
                <div className="col-span-1 lg:col-span-7 space-y-8 md:space-y-12">
                   ...
                   {selectedDev?.investmentHighlights && selectedDev.investmentHighlights.length > 0 && (
                     <div className="bg-white rounded-2xl border border-fcSlate/10 overflow-hidden">
                        <div className="px-6 py-4 border-b border-fcSlate/10 bg-fcSlate/[0.02]">
                           <div className="flex items-center gap-3">
                              <div className="p-2 bg-fcGold/10 rounded-lg">
                              ...
                              <h3 className="text-base font-semibold text-fcSlate">Key Highlights</h3>
                           </div>
                        </div>
                        <div className="p-6">
                           <div className="flex flex-wrap gap-2">
                             {selectedDev?.investmentHighlights?.map((highlight, idx) => (
                               <span 
                                 key={idx} 
                               ...
                               >
                                 <CheckCircle2 size={14} className="text-fcGold" />
                                 {highlight}
                               </span>
                             ))}
                           </div>
                   ...
                   <div className="bg-white rounded-2xl border border-fcSlate/10 overflow-hidden">
                      <div className="px-6 py-4 border-b border-fcSlate/10 bg-fcSlate/[0.02]">
                         <div className="flex items-center gap-3">
                            ...
                            <h3 className="text-base font-semibold text-fcSlate">Infrastructure Status</h3>
                         </div>
                      </div>
                      <div className="p-6">
                         {/* Infrastructure Status Grid */}
                         {(() => {
                           const estateProgress = (selectedDev as any)?.estateProgress;
                           const infraItems = [
                           ...
                           const statusConfig: Record<string, { color: string; bg: string; label: string }> = {
                             'not_started': { color: 'text-gray-500', bg: 'bg-gray-100', label: 'Pending' },
                             'planned': { color: 'text-blue-600', bg: 'bg-blue-50', label: 'Planned' },
                             'in_progress': { color: 'text-amber-600', bg: 'bg-amber-50', label: 'In Progress' },
                             'completed': { color: 'text-emerald-600', bg: 'bg-emerald-50', label: 'Complete' },
                           ...
                           return (
                             <div className="space-y-4">
                               <div className="grid grid-cols-2 gap-3">
                                 {infraItems.map((item) => {
                                   const status = estateProgress?.[item.key] || 'not_started';
                                   const config = statusConfig[status] || statusConfig['not_started'];
                                   const Icon = item.icon;
                                   return (
                                     <div key={item.key} className="flex items-center justify-between p-3 bg-fcSlate/[0.02] rounded-xl border border-fcSlate/5">
                                       <div className="flex items-center gap-3">
                                         <Icon size={16} className="text-fcSlate/40" />
                                         <span className="text-sm font-medium text-fcSlate">{item.label}</span>
                                       </div>
                               ...
                               {estateProgress?.compliance && (
                                 <div className="flex items-center justify-between p-3 bg-fcSlate/[0.02] rounded-xl border border-fcSlate/5">
                                   <div className="flex items-center gap-3">
                                     <ShieldCheck size={16} className="text-fcSlate/40" />
                                     <span className="text-sm font-medium text-fcSlate">Compliance</span>
                                   </div>
                                   <span className={`px-2.5 py-1 text-xs font-medium rounded-full ${(statusConfig[estateProgress.compliance] || statusConfig['pending']).bg} ${(statusConfig[estateProgress.compliance] || statusConfig['pending']).color}`}>
                                     {(statusConfig[estateProgress.compliance] || statusConfig['pending']).label}
                                   </span>
                                 </div>
                               )}
                             </div>
                   ...
                   <div className="bg-white rounded-2xl border border-fcSlate/10 overflow-hidden">
                      <div className="px-6 py-4 border-b border-fcSlate/10 bg-fcSlate/[0.02]">
                         <div className="flex items-center gap-3">
                            ...
                            <h3 className="text-base font-semibold text-fcSlate">Features & Amenities</h3>
                         </div>
                      </div>
                      <div className="p-6">
                         <div className="grid grid-cols-2 gap-3">
                            {(() => {
                              const features = Array.isArray((selectedDev as any)?.features) ? (selectedDev as any).features : [];
                              if (features.length > 0) {
                                return features.slice(0, 8).map((feat: string, idx: number) => (
                                  <div key={idx} className="flex items-center gap-3 p-3 bg-fcSlate/[0.02] rounded-xl border border-fcSlate/5">
                                     <CheckCircle2 size={16} className="text-fcGold shrink-0" />
                                     <span className="text-sm font-medium text-fcSlate">{feat}</span>
                                  </div>
                                ));
                              }
                              return [
                                ...
                                <div key={idx} className="flex items-center gap-3 p-3 bg-fcSlate/[0.02] rounded-xl border border-fcSlate/5">
                                   <feat.icon size={16} className="text-fcGold shrink-0" />
                                   <span className="text-sm font-medium text-fcSlate">{feat.label}</span>
                                </div>
                              ));
                            })()}
                         </div>
                   ...
                   <div className="bg-white rounded-2xl border border-fcSlate/10 overflow-hidden">
                      <div className="px-6 py-4 border-b border-fcSlate/10 bg-fcSlate/[0.02]">
                         <div className="flex items-center gap-3">
                            ...
                            <div>
                               <h3 className="text-base font-semibold text-fcSlate">Development Documents</h3>
                               <p className="text-xs text-fcSlate/50">Statutory documents & approvals</p>
                            </div>
                         </div>
                      </div>
                         ...
                         {(() => {
                           ...
                           return documentList.length > 0 ? (
                             documentList.map((doc) => (
                               <div key={doc.id} className="flex items-center justify-between p-4 bg-fcSlate/[0.02] rounded-xl border border-fcSlate/5 hover:border-fcGold/20 transition-colors group">
                                  <div className="flex items-center gap-4">
                                     <div className="p-2.5 bg-white rounded-lg border border-fcSlate/10 group-hover:border-fcGold/20 transition-colors">
                                        <FileText size={18} className="text-fcGold" />
                                     </div>
                                     <div>
                                        <h4 className="text-sm font-medium text-fcSlate">{doc.name}</h4>
                                        <p className="text-xs text-fcSlate/50">{new Date(doc.uploadedAt).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })}</p>
                                     </div>
                                  </div>
                                  <a 
                                    href={doc.url} 
                            ...
                            <div className="flex items-center gap-4 flex-1">
                               ...
                               <div className="flex-1">
                                  <div className="flex items-center gap-2 mb-1">
                                     <h4 className="text-sm font-medium text-fcSlate">Optional Insurance (Old Mutual)</h4>
                                     <span className="px-2 py-0.5 text-[10px] font-semibold text-fcGold bg-fcGold/10 rounded-full uppercase tracking-wider">Optional</span>
                                  </div>
                                  <p className="text-xs text-fcSlate/50">Enquire for more information</p>
                               </div>
                            </div>
                            <button
                               onClick={async () => {
                                  const standNumber = (selectedDev as any)?.selectedStand?.standNumber || null;
                                  const developmentName = selectedDev?.name || 'Development';
                                  ...
                                  try {
                                     const response = await fetch('/api/enquiries', {
                                        ...
                                        body: JSON.stringify({
                                           ...
                                           message,
                                           developmentId: selectedDev?.id,
                                           standId: (selectedDev as any)?.selectedStand?.id || null,
                                           standNumber,
                                           developmentName,
                                        }),
                                     });
                                     ...
                                     if (response.ok) {
                                        alert('Thank you! We will contact you with more information about Old Mutual insurance.');
                                     } else {
                ...
                <div className="col-span-1 lg:col-span-5">
                   ...
                   <div className="bg-white rounded-2xl border border-fcSlate/10 overflow-hidden lg:sticky lg:top-24">
                      <div className="px-6 py-4 border-b border-fcSlate/10 bg-fcSlate text-white">
                         <h3 className="text-base font-semibold">Investment Summary</h3>
                      </div>
                      <div className="p-6 space-y-6">
                         {/* Price Per SQM */}
                         {(() => {
                           const pricePerSqm = (selectedDev as any)?.pricePerSqm;
                           const totalArea = (selectedDev as any)?.totalAreaSqm;
                           // Safe parsing for basePrice
                           const basePrice = typeof selectedDev?.basePrice === 'number'
                             ? selectedDev.basePrice
                             : typeof (selectedDev as any)?.basePrice === 'string'
                               ? parseFloat((selectedDev as any).basePrice) || 0
                               : 0;
                           
                           let displayPricePerSqm: number | null = null;
                           // Safe parsing for pricePerSqm
                           if (pricePerSqm && typeof pricePerSqm === 'number' && pricePerSqm > 0) {
                             displayPricePerSqm = pricePerSqm;
                           } else if (typeof pricePerSqm === 'string') {
                             const parsed = parseFloat(pricePerSqm);
                             if (!isNaN(parsed) && parsed > 0) displayPricePerSqm = parsed;
                           } else if (basePrice > 0 && totalArea) {
                             // Safe parsing for totalArea
                             const areaNum = typeof totalArea === 'number' 
                               ? totalArea 
                           ...
                           return (
                             <div className="p-4 bg-fcGold/5 rounded-xl border border-fcGold/10">
                               <div className="text-xs font-medium text-fcSlate/60 mb-1">Price per Square Metre</div>
                               <div className="text-2xl font-bold text-fcSlate">
                                 {displayPricePerSqm ? (
                                   <>USD ${displayPricePerSqm.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}/m²</>
                                 ) : (
                               ...
                               {(() => {
                                 ...
                                 return areaNum > 0 ? (
                                   <div className="text-xs text-fcSlate/50 mt-1">
                                     Total area: {areaNum.toLocaleString()} m²
                                   </div>
                                 ) : null;
                               })()}
                             </div>
                         ...
                         {(() => {
                          ...
                          const hasAnyFees = fees.vatEnabled || fees.endowmentEnabled || fees.aosEnabled || fees.cessionsEnabled || fees.adminFeeEnabled;
                           
                           if (!hasAnyFees) {
                             return (
                               <div className="text-xs text-fcSlate/50">Base price only</div>
                             );
                           }
                           
                           return (
                             <div className="bg-fcSlate/5 rounded-xl p-4 space-y-2">
                               <div className="text-xs font-semibold text-fcSlate/70 mb-2">Price Breakdown</div>
                               <div className="flex justify-between text-sm">
                                 <span className="text-fcSlate/60">Base Price</span>
                                 <span className="font-medium text-fcSlate">${fees.basePrice.toLocaleString()}</span>
                               </div>
                              {fees.vatEnabled && fees.vatAmount > 0 && (
                                 <div className="flex justify-between text-sm">
                                  <span className="text-fcSlate/60">
                                   ...
                                   <span className="text-emerald-600">+${fees.vatAmount.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                                 </div>
                               )}
                               {fees.endowmentEnabled && fees.endowmentFee > 0 && (
                                 <div className="flex justify-between text-sm">
                                   <span className="text-fcSlate/60">Endowment Fee</span>
                                   <span className="text-blue-600">+${fees.endowmentFee.toLocaleString()}</span>
                                 </div>
                               )}
                               {fees.aosEnabled && fees.aosFee > 0 && (
                                 <div className="flex justify-between text-sm">
                                   <span className="text-fcSlate/60">Agreement of Sale Fee</span>
                                   <span className="text-purple-600">+${fees.aosFee.toLocaleString()}</span>
                                 </div>
                               )}
                               {fees.cessionsEnabled && fees.cessionFee > 0 && (
                                 <div className="flex justify-between text-sm">
                                   <span className="text-fcSlate/60">Cession Fee</span>
                                   <span className="text-orange-600">+${fees.cessionFee.toLocaleString()}</span>
                                 </div>
                               )}
                              {fees.adminFeeEnabled && fees.adminFee > 0 && (
                                <div className="flex justify-between text-sm">
                                  <span className="text-fcSlate/60">Admin Fee</span>
                                  <span className="text-fcGold">+${fees.adminFee.toLocaleString()}</span>
                                </div>
                              )}
                               <div className="border-t border-fcSlate/10 pt-2 mt-2">
                                 <div className="flex justify-between font-semibold text-sm">
                                   <span className="text-fcSlate">Total</span>
                                   <span className="text-fcGold">${calculateTotal(selectedDev).toLocaleString()}</span>
                                 </div>
                               </div>
                             </div>
                           );
                         ...
                         {(() => {
                           ...
                           if (selectedDev.installmentPeriods) {
                             if (Array.isArray(selectedDev.installmentPeriods)) {
                               installmentPeriods = selectedDev.installmentPeriods.filter((p: any) => typeof p === 'number' && p > 0);
                             } else if (typeof selectedDev.installmentPeriods === 'string') {
                               try {
                                 const parsed = JSON.parse(selectedDev.installmentPeriods);
                                 if (Array.isArray(parsed)) {
                           ...
                           const depositPercentage = typeof selectedDev.depositPercentage === 'number' 
                             ? selectedDev.depositPercentage 
                             : typeof (selectedDev as any)?.depositPercentage === 'string'
                             ? parseFloat((selectedDev as any).depositPercentage) || 10
                             : 10;
                           ...
                           return (
                             <div className="bg-gradient-to-br from-indigo-50 to-blue-50 rounded-xl p-4 space-y-3 border border-indigo-100">
                               <div className="flex items-center gap-2">
                                 ...
                                 <span className="text-xs font-semibold text-indigo-800">Flexible Payment Plans</span>
                               </div>
                               
                               <div className="bg-white/70 rounded-lg p-3 space-y-2">
                                 <div className="flex justify-between text-xs">
                                   <span className="text-fcSlate/60">Deposit ({depositPercentage}%)</span>
                                   <span className="font-semibold text-indigo-600">${depositAmount.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                                 </div>
                                 <div className="flex justify-between text-xs">
                                   <span className="text-fcSlate/60">Balance after deposit</span>
                                   <span className="font-medium text-fcSlate">${balance.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                                 </div>
                               </div>
                               
                               <div className="text-xs font-medium text-fcSlate/70 mb-1">Monthly payment options:</div>
                               <div className="grid grid-cols-3 gap-2">
                                 {installmentPeriods.slice(0, 3).map((months: number) => {
                                   const monthly = balance / months;
                                   return (
                                     <div key={months} className="bg-white rounded-lg p-2 text-center border border-indigo-100 hover:border-indigo-300 transition-colors">
                                       <div className="text-xs text-fcSlate/60">{months} mo</div>
                                       <div className="text-sm font-bold text-indigo-600">${monthly.toLocaleString(undefined, { maximumFractionDigits: 0 })}</div>
                                       <div className="text-[10px] text-fcSlate/50">/month</div>
                                     </div>
                                   );
                                 })}
                               </div>
                               ...
                               {installmentPeriods.length > 3 && (
                                 <div className="text-[10px] text-center text-fcSlate/50">
                                   Also available: {installmentPeriods.slice(3).map((m: number) => `${m} mo`).join(', ')}
                                 </div>
                               )}
                             </div>
                           );
              ...
              <div className="flex flex-col space-y-6 mb-12">
                <div className="flex flex-col md:flex-row md:justify-between md:items-end border-b border-gray-200 pb-8 gap-6">
                  <div className="space-y-2">
                    <p className="text-sm font-medium text-fcGold">Our Portfolio</p>
                    <h2 className="text-3xl font-bold text-gray-900">Developments</h2>
                  </div>
                  <div className="flex bg-gray-100 p-1 rounded-xl self-start md:self-auto">
                    {(['ALL', 'SERVICING', 'READY_TO_BUILD'] as const).map(p => (
                      <button 
                        key={p} 
                        onClick={() => setPhaseFilter(p)}
                        className={`px-5 py-2 rounded-lg text-sm font-medium transition-all ${
                          phaseFilter === p ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-600 hover:text-gray-900'
                        }`}
                      >
                        {p === 'ALL' ? 'All' : p === 'SERVICING' ? 'Servicing' : 'Ready'}
                      </button>
                    ))}
                  </div>
                </div>
                ...
                {filteredDevelopments.length > 0 ? (
                  filteredDevelopments.map((dev, i) => (
                    <DevelopmentCard
                      ...
                      onFavorite={() => logger.debug('Favorited development', { module: 'LandingPage', devId: dev.id })}
                      isFavorited={false}
                      index={i}
                      lazy={i > 2}
                    />
                  ...
                  <div className="col-span-full py-20 text-center">
                    ...
                    <p className="text-gray-500">No developments available for the selected filter.</p>
                  </div>
                )}
              </div>
            </>
      ...
      {/* Legal pages */}
      {activeLegalPage && (
        <LegalPages type={activeLegalPage} onClose={() => setActiveLegalPage(null)} />
      )}
